  ...
  return (
    <AppShell>
      ...
      <main className="mx-auto w-full max-w-6xl px-5 py-8 sm:px-8 sm:py-10">
        ...
        {featured && <section className="mt-9" aria-labelledby="featured-contest"><div className="mb-4 flex items-center justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-emerald-600 dark:text-emerald-400">Accepting entries</p><h2 id="featured-contest" className="mt-1 font-serif text-2xl font-bold text-[var(--text-primary)]">Featured contest</h2></div><Link href={`/contests/${featured.slug}`} className="text-xs font-bold text-[var(--accent)]">View details →</Link></div><Link href={`/contests/${featured.slug}`} className="group grid overflow-hidden rounded-3xl border border-[var(--border-default)] bg-[var(--card-bg)] transition hover:border-[var(--accent)] hover:shadow-xl md:grid-cols-[1.05fr_1fr]">{featured.coverUrl ? <img src={featured.coverUrl} alt="" className="h-64 w-full object-cover md:h-full" /> : <div className="grid min-h-56 place-items-center bg-[radial-gradient(circle_at_30%_30%,rgba(139,92,246,0.35),transparent_38%),linear-gradient(135deg,var(--bg-surface),var(--card-bg))] text-6xl text-[var(--accent)]"><ion-icon name="trophy-outline" /></div>}<div className="flex flex-col justify-center p-6 sm:p-8"><div className="flex flex-wrap items-center gap-2"><StatusBadge status={featured.status} />{featured.theme && <span className="text-[10px] font-bold uppercase tracking-[0.12em] text-[var(--text-faint)]">{featured.theme}</span>}</div><h3 className="mt-4 font-serif text-3xl font-extrabold leading-tight text-[var(--text-primary)] transition group-hover:text-[var(--accent)]">{featured.title}</h3><p className="mt-3 line-clamp-3 text-sm leading-6 text-[var(--text-muted)]">{featured.description || featured.problemStatement}</p><div className="mt-6 flex flex-wrap items-center justify-between gap-4"><Organizer contest={featured} /><span className="rounded-full bg-[var(--bg-surface)] px-3 py-1.5 text-xs font-semibold text-[var(--text-secondary)]">{featured.submissionCount} {featured.submissionCount === 1 ? 'entry' : 'entries'}</span></div><p className="mt-4 flex items-center gap-1.5 text-xs font-semibold text-emerald-600 dark:text-emerald-400"><ion-icon name="time-outline" />{timing(featured)}</p></div></Link></section>}
